                ...
                foreach ($pitem['require'] as $req) {
                    if (!in_array($req, $GLOBALS['plugins'])) {
                        ...
                        foreach ($this->download_list2 as $value) {
                            if ($value->nombre == $req) {
                                $txt .= '. Puedes activar este plugin desde la pestaña <b>plugins</b>.';
                                break;
                            }
                        }
    ...
    public function check_for_updates2()
    {
        if (!$this->user->admin) {
            ...
            if (!$updates) {
                ...
                $internet_version = @fs_file_get_contents('https://raw.githubusercontent.com/adding-tech/adding-tech/master/VERSION');
